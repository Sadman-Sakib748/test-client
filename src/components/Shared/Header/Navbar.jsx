...
import { ShoppingCart, Heart, Menu, X, Apple, AppleIcon } from "lucide-react";
...
export default function NavBar() {
  ...
  return (
    <div className={`relative ${!hideHero ?? 'min-h-screen'} bg-gradient-to-br from-green-50 to-orange-50 overflow-hidden`}>
      ...
      {/* Hero Section */}
      {!hideHero && (
        <section className="relative  py-12 lg:py-20 z-10 min-h-screen flex flex-col justify-center w-full">
          <div className="flex flex-col-reverse md:flex-row items-center md:gap-12 lg:gap-12 relative h-full w-full">
            <div className="relative w-[50px] md:-mt-[448px] h-[50px]">
              <Image
                src={tree}
                alt="Fresh Salad"
                width={50}
            ...
            <div className="flex-1 ml-13 space-y-6 md:space-y-8 lg:space-y-8 order-2 md:order-1 h-full flex flex-col justify-center relative">
              ...
              <Link href="/shop">
                <button className="w-[150px] h-[53px] bg-[#FF6A1A] hover:bg-[#e65c00] text-white rounded-[8px] pt-4 pb-4 pl-8 pr-8 text-sm md:text-lg lg:text-lg">
                  Shop Now
                </button>
              </Link>
